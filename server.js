...
// Enable serving static files from 'saved_notes' directory
...
        const [result] = await pool.execute('INSERT INTO documents (user_id, type, content, file_path) VALUES (?, ?, ?, ?)', [userId, 'speech_to_text', note, `saved_notes/${fileName}`]);
...
// API to get all non-deleted notes for a user
app.get('/notes/:userId', async (req, res) => {
    const { userId } = req.params;
    try {
        const [rows] = await pool.execute('SELECT id, content, file_path AS filePath, created_at AS createdAt FROM documents WHERE user_id = ? AND deleted_at IS NULL ORDER BY created_at DESC', [userId]);
...
// API to soft delete a note and log to history
